...
public:
  ...
  void optimize()
  {
    for (int generation = 0; generation < max_generations; ++generation)
    {
      for (int i = 0; i < population_size; ++i)
      {
        // Select 6 distinct random individuals different from i and best_index
        ...
        // Perform DE/best/3 mutation
        // x_best + F * (x_r1 - x_r2 + x_r3 - x_r4 + x_r5 - x_r6)
        Solution mutant(dimension);
        for (int j = 0; j < dimension; ++j)
        {
          mutant[j] = population[best_index][j] + F * (population[indices[0]][j] - population[indices[1]][j] +
                                                       population[indices[2]][j] - population[indices[3]][j] +
                                                       population[indices[4]][j] - population[indices[5]][j]);
...
int main()
{
  ...
  std::cout << "Starting Differential Evolution (DE/best/3) optimization..." << std::endl;
  ...
  std::cout << "Best fitness: " << std::scientific << std::setprecision(10) << de.get_best_fitness() << std::endl;
